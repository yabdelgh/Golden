...
import { Shape, moveDetail } from "../shapes/shape";
...
export abstract class APlayer {
    ...
    movingDirection: PlayerMove = PlayerMove.Left 
    ...
    private update_padel_position() {
        let pos_x
        if (!this.player_side || !this._game_size)
            return;
        if (this.player_side == PlayerMove.Left) {
            ...
            pos_x = this._game_size.x - this._padel_gap - GetBodySize(this.body).x / 2;
            Body.setPosition(this.body, Vector.create(pos_x, this._game_size.y / 2))
        }
    }
    ...
    public get goal(): Body {
        if (!this._goal) {
            ...
            if (this._player_side == PlayerMove.Left)
                ...
                this._goal = Bodies.rectangle(this._game_size.x, this._game_size.y / 2, 1, this._game_size.y)
